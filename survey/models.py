...
class Survey(models.Model):
    ...
    submitted_at = models.DateTimeField(null=True)  # Add this field
        ...
        if self.question.question_type == Question.FILE:
            ...
            if self.text or self.choice:
                raise ValidationError(
                    'Text and choice fields must be empty for file type questions')
            ...
            if not self.file.name.endswith(self.question.file_format):
                raise ValidationError(
                    'File format must be ' + self.question.file_format)
